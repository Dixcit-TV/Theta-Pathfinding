...
namespace Elite 
{
	...
	/*! Project point on line segment. If not on line, remap to fit on line segment (with or without extra offset)*/
	inline Vector2 ProjectOnVector(const Vector2& startPoint, const Vector2& vector, const Vector2& point)
	{
		//Projection
		auto w = point - startPoint;
		auto proj = Dot(w, vector);
		//auto offsetPercentage = offset / line.Magnitude();
		...
		if (proj <= 0)
			...
			return startPoint + (proj / Dot(vector, vector)) * vector.GetNormalized();
	}

	inline bool IsSegmentIntersectingWithCircle(const Vector2& startSegment, const Vector2& endSegment, const Vector2& circleCenter, float circleRadius)
	{
	...
	inline bool IsVectoIntersectingWithCircle(const Vector2& startPoint, const Vector2& direction, const Vector2& circleCenter, float circleRadius)
	{
		Vector2 closestPoint = ProjectOnVector(startPoint, direction, circleCenter);
		Vector2 distanceToCircleCenter = circleCenter - closestPoint;
